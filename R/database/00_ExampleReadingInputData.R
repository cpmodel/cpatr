##-----------------------------------------------------------------------------
# Name:         00_ExampleReadingInputData
# Description:  A set quick blocks of code to read each type of input data
##-----------------------------------------------------------------------------
...
# Storing all relevant lookups from centralized file in a list
Lookups                 <- list()


# Notice the working directory is in the root of the data folder.
# If not, the path may need to be adapted (for instance, '../../data/Lookups')
Lookups$CountryCode     <- read_excel("Lookups/Lookups.xlsx", sheet = "CountryCode")
Lookups$EFCompleting    <- read_excel("Lookups/Lookups.xlsx", sheet = "EmissionFactorsCompleting")
Lookups$LargeFuelList   <- read_excel("Lookups/Lookups.xlsx", sheet = "FuelCodes")
Lookups$FuelTypes       <- read_excel("Lookups/Lookups.xlsx", sheet = "FuelTypes")
Lookups$PowFuelTypes    <- read_excel("Lookups/Lookups.xlsx", sheet = "PowerFuelTypes")
Lookups$ProjFuelTypes   <- read_excel("Lookups/Lookups.xlsx", sheet = "ProjectionsFuelTypes")
Lookups$SectorGroups    <- read_excel("Lookups/Lookups.xlsx", sheet = "FlowSectorGroupMapEC")
...
# NOTE: Full country list that also includes WORLDAV and WORLDMAR.
#       This extended list is used to create the list of potential coverage.
#       Actual coverage differs and the resulting list of countries will be shorter
...
#  This function:
#   - Imports and arranges pre-processed EC data to fit in required format
#   - The results are used to provide base year for EC projections (hindcasting or forecasting)
...
PrepareObsEC    <- function(InitYear  = StartYear,
  ...
  # NOTE:
  # EC data already includes a combination of countries (for which it has coverage), sectors and fuels.
  # EC is taken to define the coverage of the mitigation equation, so no further expansion is required.

  TempEC.id         <- TempEC %>% select(-as.character(YearVector), -Variable)
  TempEC.matrix     <- as.matrix(TempEC %>%
                                   select(as.character(YearVector)))
  EC.obs            <- tibble(TempEC.id, ec.obs = TempEC.matrix)
...
PrepareHistPrices   <- function(InitYear   = StartYear,
  ...
  # NOTE: Prices for wnd sol hyd ore nuc are later on set to 1 as placeholder
