...
PrepareGDPRelativeToBase <- function(DD    = D,
                                     BaseL = BaseList,
                                     LU    = BaseList$LU){
...
PrepocessIntPricesList      <- function(BaseL = BaseList,
                                        LU    = BaseList$LU){
    ...
    IntPr_RegAssum  <- read_excel('data/Prices/1_Raw/RInputs_InternationalPrices.xlsx', sheet = 'RegionAssumptions') %>%
                        select(-FuelName) %>%
    ...
    TempStep1       <- BaseL$IDcols %>%
                        left_join(Lookups$CountryCode, by = 'CountryCode') %>%
    ...
    IntPr_RegMarket   <- read_excel('data/Prices/1_Raw/RInputs_InternationalPrices.xlsx', sheet = 'RegionMarket') %>%
                          select(-Country) %>%
    ...
    RawStep2            <- read_excel('data/Prices/1_Raw/RInputs_InternationalPrices.xlsx', sheet = 'IntPrices') %>%
...
PrepareInternationalPrices      <- function(DD        = DL$Scenario1,
                                            BaseL     = BaseList,
                                            IPList    = PrepocessIntPricesList(),
                                            MTI       = MTinputsList$Scenario1,
                                            SelSource = 'IMF-IEA'){
